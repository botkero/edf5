...
with open(f"missions/index.md", "w") as f:
    f.write("# Missions\n")
    for counter, mission in enumerate(missions):
        # header
        f.write(f"""## {counter}. {mission["value"][0]["value"]}\n\n""")

        # image
        f.write("<figure markdown>\n")
        filename = format_filename(mission["value"][0]["value"])
        image = f"""  [![{mission["value"][0]["value"]}]""" + dlc2[counter] + """{ .off-glb style="width: 65vh;" loading=lazy }]""" + f"""({filename}.md)"""
        f.write(image+"\n")
        f.write("</figure>\n\n")
